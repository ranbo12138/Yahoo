...
class MainActivity : AppCompatActivity() {
    ...
    private val camL = registerForActivityResult(ActivityResultContracts.TakePicturePreview()) { 
        it?.let { proc(it) } 
    }
    ...
    private val galL = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        it.data?.data?.let { u -> 
            try { 
                proc(MediaStore.Images.Media.getBitmap(contentResolver, u)) 
            } catch (_: Exception) { 
                toast("读取失败") 
            } 
        }
    ...
    private val camP = registerForActivityResult(ActivityResultContracts.RequestPermission()) { 
        if (it) camL.launch(null) 
    }
    ...
    private fun reqCap() {
        ...
        capL.launch(pm.createScreenCaptureIntent())
    }
    ...
    private fun startSvc(code: Int, data: Intent) {
        ...
        scope.launch { 
            delay(1000)
            updBtn()
            toast(if (ScreenCaptureService.isRunning) "✅服务启动！切换漫画后返回点击截取" else "❌启动失败")
        }
    }
    ...
    private fun updBtn() { 
        btnCap.text = if (ScreenCaptureService.isRunning) "📷截取屏幕" else "截屏翻译" 
    }
        ...
        scope.launch {
            try {
                ...
                if (txt.isBlank()) { 
                    ...
                    input.setText(filter(txt))
                    result.text = ""
                    toast("识别成功") 
                }
            } catch (e: Exception) { 
                Logger.log("proc异常: ${e.message}")
                toast("识别失败")
                result.text = "" 
            }
        }
    ...
    private fun translate() {
        ...
        scope.launch {
            try {
                ...
                val req = ChatRequest(model, listOf(
